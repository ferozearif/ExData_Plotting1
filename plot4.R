...
# Reads data from a filtered file and generates four plots
...
# Use par to ensure that subsequent plots are drawn in an array of 2 columns by 2 rows
par(mfrow=c(2,2))
...
plot(strptime(paste(hpc$Date, hpc$Time), "%d/%m/%Y %H:%M:%S"), hpc$Voltage, type = "l", ylab="Voltage", xlab="datetime")
...
plot(strptime(paste(hpc$Date, hpc$Time), "%d/%m/%Y %H:%M:%S"), hpc$Global_reactive_power, type = "l", ylab="Global_reactive_power", xlab="datetime")
# Close device
dev.off()